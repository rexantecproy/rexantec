    ...
    return <ul className='w-full flex flex-col'>
...

        {
            props.sub_content_1.map((sub_cot, i) => {
                return <li key={`$ID-${i}---${sub_cot.name}`} className={`${isOpenSubList ? '' : 'hidden'} ${i == 0 ? 'mt-3' : ''} w-full [&_a]:inline-block [&_a]:w-full border-b-2 border-black/0 hover:border-b-2 hover:border-sky-500 hover:text-sky-500 hover:bg-sky-100 cursor-pointer`}>
